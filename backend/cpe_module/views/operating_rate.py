...
def resolve_station_id(settings):
    ...
    if reference_year == year:
        ...
        for h_date in all_holiday_dates:
            try:
                ...
                # 2월 29일 처리: 윤년이 아닌해로 투영 시 에러 발생 -> 2월 28일로 매핑하거나 제외
                # 여기서는 2월 28일로 매핑
...
def calculate_operating_rates(project_id, weights, settings):
    station_id = resolve_station_id(settings)
    ...
    if station_id:
        ...
        for weight in weights:
            ...
            if holiday_years.exists():
                holiday_stats = []
                for h_year in sorted(set(holiday_years))[:years]:
...
def update_work_schedule_weight(request, project_id):
    ...
    for item in weights_data:
        ...
        if serializer.is_valid():
            weight = serializer.save()
            updated_items.append(weight)
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    # 계산 Logic Invoke
    calculate_operating_rates(project_id, updated_items, settings)
